# **Privacy Policy for Smart Clipboard**

_Last updated: 24 dec 2024_

Thank you for using **Smart Clipboard** (“the App”).
This Privacy Policy explains how your data is collected, used, stored, and protected when you use the App.

By using Smart Clipboard, you agree to the terms described here.

---

## 📌 **1. Information We Collect**

### **1.1 Local Data (No Login)**
If you use the App without logging in, all your data is stored **locally on your device only**.  
This may include:
- Clipboard items  
- Templates 
- App preferences  

➡️ This data **never leaves your device** unless you uninstall the app or manually delete it.

---

### **1.2 Cloud Data (Logged-in Users)**
If you log in using **Google Sign-In**, we collect minimal information required to sync your data:

#### Collected Information:
- Your **Google account ID** (unique user identifier)
- Your templates and clipboard items

#### Purpose:
- To sync your data across devices
- To identify your account on Firebase
- To securely store cloud data

---

## 📌 **2. How We Use Your Information**

We use your information to:
- Provide cloud sync for your templates and clipboard items
- Keep your data associated with your account  
- Improve app performance and reliability  

We **do NOT**:
- Sell your data  
- Share your data with third parties  
- Use your data for advertising  

---

## 📌 **3. Data Deletion**

You may delete your data at any time.

### **3.1 Local Data**
- Delete templates or items manually within the app  
- Uninstalling the app removes all local data automatically  

### **3.2 Cloud Data (Firebase)**
Logged-in users can request full deletion of their account data.

You can request deletion:
- From within the app (In Profile section), or  
- By contacting us at: **therapidcoders@gmail.com**

Once requested:
- All your cloud data will be permanently removed from Firebase

---

## 📌 **4. Third-Party Services Used**

The app uses:
- **Google Sign-In**
- **Firebase Authentication**
- **Firebase Firestore**

---

## 📌 **5. Children's Privacy**

Smart Clipboard is **not intended for children under 13**.  
We do not knowingly collect data from children under 13.


---

## 📌 **6. Changes to This Privacy Policy**

We may update this Privacy Policy occasionally.  
Changes will be posted with a new “Last Updated” date.

---

## 📌 **7. Contact Us**

If you have any questions about this Privacy Policy or want to request data deletion, contact us at:

**Email:** therapidcoders@gmail.com
